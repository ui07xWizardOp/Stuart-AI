...
```mermaid
graph TD
    subgraph "Interface Layer (Cognitive Terminal)"
        A[Bento Dashboard] --> B[Ethereal Grid Redesign]
        B --> C[Glassmorphic HIL Panel]
    end

    subgraph "Orchestration Layer"
        D[FastAPI Backend] --> E[Agent Runtime]
        E --> F[ReAct Loop Engine]
        F -- Capability Tokens --> G[Security Guards]
    end

    subgraph "Cognitive Layer"
        H[Long-term Memory SQL] --> I[Distillation Engine]
        I --> J[Insight Extraction]
    end

    subgraph "Stealth Interaction Layer"
        K[Win32 API Hook] --> L[WDA_EXCLUDEFROMCAPTURE]
        L --> M[Global Hotkey Listener]
    end
