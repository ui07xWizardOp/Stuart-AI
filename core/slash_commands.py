...
class SlashCommandRouter:
    ...
    def _cmd_telos(self, args: str) -> str:
        ...
        if not telos:
            return "⚠️ TELOS Framework is not initialized."
        ...
        return "⚠️ Invalid syntax. Use `/telos` to view, or `/telos update <text>` to change."

    def _cmd_skills(self, args: str) -> str:
        """Manage community skill plugins."""
        marketplace = self._context.get("skills_marketplace")
        if not marketplace:
            return "⚠️ Skills Marketplace is not initialized."
        ...
        if action == "list":
            ...
            for s in skills:
                status = "✅ Installed" if s["installed"] else "⬇️ Available"
            ...
            if not name:
                return "⚠️ Specify a skill name: `/skills install <name>`"
            return marketplace.install_skill(name)

        elif action == "remove":
            if not name:
                return "⚠️ Specify a skill name: `/skills remove <name>`"
            return marketplace.remove_skill(name)

        return "⚠️ Unknown action. Use `/skills list`, `/skills install <name>`, or `/skills remove <name>`."

    def _cmd_traces(self, args: str) -> str:
        """Show tracing statistics."""
        tracing = self._context.get("tracing")
        if not tracing:
            return "⚠️ Tracing system is not initialized."
        ...
        return "⚠️ Use `/traces stats` or `/traces recent`."
    ...
    def _cmd_model(self, args: str) -> str:
        ...
        if not router:
            return "⚠️ Model router not available."
        ...
        try:
            ...
            return f"⚠️ Could not fetch router status: {e}"

    def _cmd_autonomy(self, args: str) -> str:
        """Change or display autonomy level."""
        approval = self._context.get("approval_system")
        if not approval:
            return "⚠️ Approval system not available."
    ...
    def _cmd_cron(self, args: str) -> str:
        ...
        cron = self._context.get("cron_manager")
        if not cron:
            return "⚠️ Cron manager not available."
    ...
    def _cmd_clear(self, args: str) -> str:
        ...
        if not memory:
            return "⚠️ Memory system not available."

        try:
            memory.short_term.clear()
            return "🗑️ Short-term memory cleared."
        except Exception as e:
    ...
    def _cmd_plan(self, args: str) -> str:
        ...
        if not plan_lib:
            return "⚠️ Plan library not available."
        ...
        try:
            ...
            return f"⚠️ Could not list plans: {e}"

    def _cmd_budget(self, args: str) -> str:
        """Show token quota dashboard."""
        quota = self._context.get("token_quota")
        if not quota:
            return "⚠️ Token quota not available."

        try:
            status = quota.get_status()
            daily_used = status.get("daily_used", 0)
            daily_limit = status.get("daily_limit", 0)
            session_used = status.get("session_used", 0)
            session_limit = status.get("session_limit", 0)
            cloud_used = status.get("cloud_daily_used", 0)
            cloud_limit = status.get("cloud_daily_limit", 0)
            ...
            return (
                ...
                f"  ☁️ Cloud:   {bar(cloud_pct)}  ({cloud_used:,} / {cloud_limit:,})\n"
            )
        except Exception as e:
            return f"⚠️ Could not fetch budget: {e}"
    ...
    def _cmd_memory(self, args: str) -> str:
        ...
        orchestrator = self._context.get("orchestrator")
        if not orchestrator:
            return "⚠️ Orchestrator not available for consolidation."
